...
public class ProxyingHttpCommandExecutor implements CommandExecutor, NeedsLocalLogs {
    ...
    public ProxyingHttpCommandExecutor(URL addressOfRemoteServer) {
        this(ImmutableMap.<String, CommandInfo>of(), addressOfRemoteServer);
    }

    public ProxyingHttpCommandExecutor(Map<String, CommandInfo> additionalCommands, URL addressOfRemoteServer) {
        try {
        ...
        synchronized (ProxyingHttpCommandExecutor.class) {
    ...
    public Response execute(Command command) throws IOException {
        ...
        // Get any system settings which add the pre-emptive proxy auth header
        String proxyUser = System.getProperty("http.proxyUser");
        String proxyPassword = System.getProperty("http.proxyPassword");

        if (StringUtils.isNotEmpty(proxyUser)) {
            String authEncoded = new String(Base64.encodeBase64((proxyUser + ":" + proxyPassword).getBytes()), StandardCharsets.UTF_8);
            httpMethod.addHeader("Proxy-Authorization", "Basic " + authEncoded);
        }
